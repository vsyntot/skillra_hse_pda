...
def test_build_skill_demand_profile_ignores_noisy_flags():
    df = pd.DataFrame(
        {
    ...
    demand = personas.build_skill_demand_profile(df, persona, min_share=0.0)

    assert "skill_sql" in demand["skill_name"].values
    assert not {"has_metro", "has_test_task", "has_php"} & set(demand["skill_name"])


def test_skill_gap_for_persona_drops_web_noise_for_data_personas():
    df = pd.DataFrame(
        {
    ...
    assert "has_php" not in gap["skill_name"].values
    recommended = gap.loc[gap["gap"], "skill_name"].tolist()
    assert "has_php" not in recommended
    assert "has_python" in recommended
