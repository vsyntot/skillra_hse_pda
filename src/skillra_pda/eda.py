...
def extract_primary_domain(
    row: pd.Series, domain_cols: list[str], priorities: list[str] | None = None
) -> str:
    """Select the primary domain from boolean domain_* flags with optional priorities."""

    # prioritize explicit order if provided
    if priorities:
        for col in priorities:
            if col in domain_cols and bool(row.get(col, False)):
...
def describe_salary_by_domain(df: pd.DataFrame, salary_col: str = "salary_mid_rub_capped") -> pd.DataFrame:
    ...
    priorities = [c for c in domain_cols if "finance" in c]  # example priority if present
    temp = df.copy()
    temp["domain"] = domain_flags.apply(extract_primary_domain, axis=1, domain_cols=domain_cols, priorities=priorities)
...
def top_employers(
    df: pd.DataFrame, company_col: str = "company", salary_col: str = "salary_mid_rub_capped", top_n: int = 10
) -> pd.DataFrame:
    """Top employers by vacancy count with median salary."""

    required = {company_col, salary_col}
    if missing := required - set(df.columns):
        raise ValueError(f"top_employers: expected columns {missing}")

    grouped = (
        df.groupby(company_col)[salary_col]
...
def benefits_by_employer(df: pd.DataFrame, company_col: str = "company", top_n: int = 10) -> pd.DataFrame:
    """Pivot of benefit shares for top employers (employer x benefit_*)."""
    ...
    pivot = subset.groupby(company_col)[benefits].mean()
    pivot["n_vacancies"] = subset.groupby(company_col).size()
...
# --- Supplemental employer-level helpers (appended without modifying existing logic) ---
_original_top_employers = top_employers
_original_benefits_by_employer = benefits_by_employer
_original_soft_skills_by_employer = soft_skills_by_employer


def top_employers(df: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
    """Топ работодателей по числу вакансий и медианной зарплате."""

    return _original_top_employers(
        df=df, company_col="company", salary_col="salary_mid_rub_capped", top_n=top_n
    )


def benefits_by_employer(df: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
    """Доли бенефитов для топовых работодателей (pivot работодатель × benefit_)."""

    return _original_benefits_by_employer(df=df, company_col="company", top_n=top_n)


def soft_skills_by_employer(df: pd.DataFrame, top_n: int = 10) -> pd.DataFrame:
    """Доли soft-навыков для топовых работодателей (pivot работодатель × soft_)."""

    return _original_soft_skills_by_employer(df=df, company_col="company", top_n=top_n)
