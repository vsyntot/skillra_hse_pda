...
def benefits_employer_heatmap(
    df: pd.DataFrame, top_n: int = 10, figsize: tuple[int, int] = (10, 6), savepath: Path | None = None
) -> Path:
    """Build a heatmap of benefit prevalence for top employers."""
    ...
    benefit_cols = [c for c in pivot.columns if c.startswith("benefit_")]
    index_col = next((c for c in pivot.columns if c not in benefit_cols and c != "n_vacancies"), None)
    if index_col is None or not benefit_cols:
        raise ValueError("benefits_employer_heatmap: no benefit columns to plot")

    matrix = pivot.set_index(index_col)[benefit_cols].fillna(0.0).astype(float)
    fig, ax = plt.subplots(figsize=figsize)
    cax = ax.imshow(matrix.values, aspect="auto", cmap="Greens")
...
def soft_skills_employer_heatmap(
    ...
    """Build a heatmap of soft-skill prevalence for top employers."""
    ...
    soft_cols = [c for c in pivot.columns if c.startswith("soft_")]
    index_col = next((c for c in pivot.columns if c not in soft_cols and c != "n_vacancies"), None)
    if index_col is None or not soft_cols:
        raise ValueError("soft_skills_employer_heatmap: no soft skill columns to plot")

    matrix = pivot.set_index(index_col)[soft_cols].fillna(0.0).astype(float)
    fig, ax = plt.subplots(figsize=figsize)
    cax = ax.imshow(matrix.values, aspect="auto", cmap="Purples")
    ax.set_xticks(range(len(matrix.columns)))
    ax.set_xticklabels(matrix.columns, rotation=90)
    ax.set_yticks(range(len(matrix.index)))
    ax.set_yticklabels(matrix.index)
...
# --- Supplemental employer heatmaps (appended without touching existing implementations) ---
_existing_benefits_employer_heatmap = benefits_employer_heatmap
_existing_soft_skills_employer_heatmap = soft_skills_employer_heatmap


def benefits_employer_heatmap(
    df: pd.DataFrame, top_n: int = 10, figsize: tuple[int, int] = (10, 6), savepath: Path | None = None
) -> Path:
    """Heatmap долей бенефитов по работодателям (top_n), сохраняет в reports/figures."""

    return _existing_benefits_employer_heatmap(df=df, top_n=top_n, figsize=figsize, savepath=savepath)


def soft_skills_employer_heatmap(
    df: pd.DataFrame, top_n: int = 10, figsize: tuple[int, int] = (10, 6), savepath: Path | None = None
) -> Path:
    """Heatmap долей soft-навыков по работодателям (top_n), сохраняет в reports/figures."""

    return _existing_soft_skills_employer_heatmap(df=df, top_n=top_n, figsize=figsize, savepath=savepath)
