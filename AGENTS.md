...
2. В `CODEx_PLAN.md` зафиксированы:
   - контекст проекта,
   - архитектура кода,
   - история шагов **Step 0–10** (уже выполнены),
   - открытые шаги **Step 11** и **Step 12**.
3. Codex **НЕ должен пытаться выполнить все шаги сразу**: пользователь всегда явно указывает, какой `Step N` выполнять.

Если текущий код/файлы противоречат актуальным инструкциям в `CODEx_PLAN.md` или сообщениям пользователя,
...
1. Шаги **Step 0–10** из `CODEx_PLAN.md` завершены.
2. Эти шаги служат историей развития проекта и справкой по архитектуре/целям.
3. Codex:
   ...
   - может вносить только **инкрементальные изменения** (добавить график, EDA‑блок, docstring, markdown и т.п.).
...
Итоговый статус шагов:
- **Step 0–10** — выполнены;
- **Step 11** — открыт (docs cleanup и стабилизация плана);
- **Step 12** — открыт (investor story и визуализации).
