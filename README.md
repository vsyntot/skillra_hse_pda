...
- Скрипт прогоняет пайплайн, выполняет ноутбук и собирает HTML с подключёнными стилями из `reports/notebooks/style.css` (код и ячейки ввода скрыты).
- Все графики из `reports/figures/` встраиваются внутрь, поэтому итоговый файл не зависит от внешних путей.
- Готовый отчёт сохраняется в `reports/notebooks/01_hse_project.html` (директория создаётся автоматически).
...
2. Выполнить ноутбук: открыть `notebooks/01_hse_project.ipynb` и прогнать все ячейки или вызвать `python scripts/validate_notebook.py` для headless-выполнения (графики сохраняются в `reports/figures/`).
3. Собрать презентационный отчёт: `python scripts/validate_notebook.py` генерирует `reports/notebooks/01_hse_project.html` с встроенными графиками без кода.
