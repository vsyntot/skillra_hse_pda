...
## HTML-отчёт
- Сгенерировать презентационную версию ноутбука: `python scripts/validate_notebook.py` (скрипт выполнит пайплайн, прогонит ноутбук и соберёт HTML без кода).
- Файл сохраняется в `reports/notebooks/01_hse_project.html`.
- Отчёт предназначен для демонстрации EDA и продуктовых выводов инвесторам, руководителям и преподавателю без необходимости запускать ноутбук вручную.
