...
- текстовые маркеры неопределённости (`unknown`, `не указано`, пустые строки, `n/a` и т.п.) нормализуются в `NaN`,
  затем категориальные признаки заполняются значением `"unknown"` для сохранения типа;
- булевы/числовые столбцы остаются в своих типах (`boolean`/`float`), где `unknown` трактуется как `NaN`;
- ключевые рыночные метрики учитывают долю `unknown`: у `work_mode` ≈34% строк без явного формата, у уровня английского ~3.5% строк не позволяют определить уровень, по образованию явных указаний нет (см. разделы ниже).
